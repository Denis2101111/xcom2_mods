...
class X2Effect_WOTC_APA_Class_ConditionalHitModifier extends X2Effect_Persistent;
	...
	if (!bApplyAsTarget)
	{
		for (i = 0; i < Modifiers.Length; ++i)
		{
			...
			for (j = 0; j < ToHitConditions.Length; ++j)
			{
				...
				if (AvailableCode != 'AA_Success')
					break;
				AvailableCode = ToHitConditions[j].AbilityMeetsCondition(AbilityState, Target);
				if (AvailableCode != 'AA_Success')
					break;
			}
	...
	if (bApplyAsTarget)
	{
		for (i = 0; i < Modifiers.Length; ++i)
		{
			...
			for (j = 0; j < ToHitConditions.Length; ++j)
			{
				...
				if (AvailableCode != 'AA_Success')
					break;
				AvailableCode = ToHitConditions[j].CallAbilityMeetsCondition(AbilityState, Target);
				if (AvailableCode != 'AA_Success')
					break;
			}
