...
class X2Effect_WOTC_APA_Class_ModifyAbilityCharges extends X2Effect_Persistent;
...
simulated protected function OnEffectAdded(const out EffectAppliedData ApplyEffectParameters, XComGameState_BaseObject kNewTargetState, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	...
	foreach TargetUnit.Abilities(AbilityStateRef)
	{
		...
		if (AbilitiesToModify.Find(AbilityTemplate.DataName) != -1)
		{
			...
			/**/`Log("WOTC_APA_Class - ModifyAbilityCharges: Current Ammo/Charges for" @ AbilityTemplate.LocFriendlyName @ "are" @ CurrentCharges, bLog);
			/**/`Log("WOTC_APA_Class - ModifyAbilityCharges: Ammo/Charge modifier is:" @ iChargeModifier $ ", Minimum Charges are" @ iMinCharges $ ", Maximum Charges are" @ iMaxCharges, bLog);
			/**/`Log("WOTC_APA_Class - ModifyAbilityCharges: Setting Ammo/Charges for" @ AbilityTemplate.LocFriendlyName @ "to" @ NewCharges, bLog);
