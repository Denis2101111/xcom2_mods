...
class X2Effect_WOTC_APA_ApexPredator extends X2Effect_Persistent;

var name WOTC_APA_ApexPredator_TriggeredName;
...
function int GetAttackingDamageModifier(XComGameState_Effect EffectState, XComGameState_Unit Attacker, Damageable TargetDamageable, XComGameState_Ability AbilityState, const out EffectAppliedData AppliedData, const int CurrentDamage, optional XComGameState NewGameState) 
{
	...
	if (NewGameState != none)
	{
		...
		if (AbilityState.SourceWeapon == Attacker.GetItemInSlot(eInvSlot_PrimaryWeapon).GetReference())
		{
			...
			{
				`XEVENTMGR.TriggerEvent(default.WOTC_APA_ApexPredator_TriggeredName, XComGameState_Unit(TargetDamageable), Attacker, NewGameState);
	}	}	}
...
DefaultProperties
{
	WOTC_APA_ApexPredator_TriggeredName = "WOTC_APA_ApexPredator_Triggered"
