...
class X2Effect_WOTC_APA_ProtocolDetectionPenalty extends X2Effect_PersistentStatChange;
...
// Remove this Detection Modifier penalty when unit concealment is lost (so it is reset if the unit regains concealment)
static function EventListenerReturn RemoveEffectOnReveal(Object EventData, Object EventSource, XComGameState NewGameState, Name InEventID, Object CallbackData)
{
	...
	`LOG("WOTC_APA_Specialist - RemoveEffectOnReveal: Expected(CallbackData) Source Unit" @ ExpectedSourceUnit.GetFullName(), bLog);

	// Get the Source Units for the Event
	SourceUnit = XComGameState_Unit(EventSource);
	`LOG("WOTC_APA_Specialist - RemoveEffectOnReveal: (EventSource)Source Unit" @ SourceUnit.GetFullName(), bLog);

	// Check that the SourceUnit is the Expected SourceUnit
	if (ExpectedSourceUnit.ObjectID != SourceUnit.ObjectID)
	{
		`LOG("WOTC_APA_Specialist - RemoveEffectOnReveal: Event does not originate from Expected SourceUnit.", bLog);
		return ELR_NoInterrupt;
	}
	`LOG("WOTC_APA_Specialist - RemoveEffectOnReveal: Expected SourceUnit found.", bLog);
