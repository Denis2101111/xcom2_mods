...
class X2TargetingMethod_WOTC_APA_ZoneSuppression extends X2TargetingMethod;


var XCom3DCursor Cursor;
var XComPrecomputedPath GrenadePath;
var vector NewTargetLocation, FiringLocation, AimingLocation;
var bool bRestrictToSquadsightRange;
var StateObjectReference AssociatedPlayerStateRef;
var float ConeLength, ConeWidth;
var XComWorldData WorldData;

var X2Actor_ConeTarget ConeActor;
var bool bGoodTarget;
var TTile FiringTile;

var array<TTile>			ReticuleTargetedTiles;    // These are tiles the targeting system has set to be active tiles
var array<TTile>			ReticuleTargetedSecondaryTiles;    // These are neighboring tiles the from the main targeted tiles

var float AimZOffset;
...
function Init(AvailableAction InAction, int NewTargetIndex)
{
	...
	ShootAtLocation = FiringUnit.GetShootAtLocation(eHit_Success, FiringUnit.GetVisualizedStateReference());
	AimZOffset = ShootAtLocation.Z - WorldData.GetFloorZForPosition(FiringUnit.Location, true);
}
...
function Update(float DeltaTime)
{
	...
	AimingLocation = FiringLocation + (Direction / VSize(Direction)) * ConeLength;       //  recalibrate based on direction to cursor; must always be as long as possible
	AimingLocation.Z = WorldData.GetFloorZForPosition(AimingLocation, true) + AimZOffset;
...
//	`SHAPEMGR.DrawSphere(TargetLocations[0], vect(15, 15, 15), MakeLinearColor(0, 0, 1, 1), true);
//	`SHAPEMGR.DrawSphere(NewTargetLocation, vect(15, 15, 15), MakeLinearColor(1, 0, 0, 1), true);
}
...
// Prevent highlighting any targets that are not in LOS
protected function GetTargetedActors(const vector Location, out array<Actor> TargetActors, optional out array<TTile> TargetTiles)
{
	...
	foreach TargetActors(CurrentTarget)
	{
		TargetedUnit = XGUnit(CurrentTarget);
		if (TargetedUnit == none)
		{
			TargetActors.RemoveItem(CurrentTarget);
		}
		else
		{
			if (`TACTICALRULES.VisibilityMgr.GetVisibilityInfo(FiringUnit.ObjectID, TargetedUnit.ObjectID, VisInfo))
			{
				if (!VisInfo.bVisibleBasic)
				{
					TargetActors.RemoveItem(CurrentTarget);
	}	}	}	}
...
//// Set PrimaryTarget from Multi-Target list
...
//{
    ...
    //{
        //AdditionalTargets.PrimaryTarget = AdditionalTargets.AdditionalTargets[0];
		//UnitState.m_MultiTurnTargetRef = AdditionalTargets.AdditionalTargets[0];
    //}
    //return true;
//}