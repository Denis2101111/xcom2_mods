...
class X2Effect_WOTC_APA_RemoteStart extends X2Effect;
...
simulated protected function OnEffectAdded(const out EffectAppliedData ApplyEffectParameters, XComGameState_BaseObject kNewTargetState, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	...
	for (i = 0; i < Actor.DestroyedEvents.Length; ++i)
	{
		if (Actor.DestroyedEvents[i].Action != None)
		{
			...
			{
				`LOG("WOTC_APA_RemoteStart - DamageAction.DamageTileRadius = " @ DamageAction.DamageTileRadius);
				DamageAction.DamageTileRadius *= class'X2Ability_ReaperAbilitySet'.default.RemoteStartRadiusMultiplier;
				`LOG("WOTC_APA_RemoteStart - DamageAction.DamageTileRadius = " @ DamageAction.DamageTileRadius);
			}
			...
			{
				EffectCueAction.ScaleAdjustment *= class'X2Ability_ReaperAbilitySet'.default.RemoteStartRadiusMultiplier;
			}
		}
	}
