...
class X2Effect_WOTC_APA_FleetFootedEmergencyAidActionPoint extends X2Effect_Persistent;
...
{
	// If the Emergency Aid effect has already been consumed, always return false
	if (SourceUnit.AffectedByEffectNames.Find('Medikit_FreeAction') == -1)
		return false;

	/**/`Log("WOTC_APA_Medic - Fleet-Footed Emergency Aid: PostAbilityCostPaid check for " $ SourceUnit.GetFullName());
	/**/`Log("WOTC_APA_Medic - Fleet-Footed Emergency Aid: Number of Action Points = " $ SourceUnit.NumActionPoints());
	/**/`Log("WOTC_APA_Medic - Fleet-Footed Emergency Aid: PostAbility = " $ kAbility.GetMyTemplateName());
	...
	if (SourceUnit.NumActionPoints() == 0)
		if (kAbility.GetMyTemplateName() == 'StandardMove')
		{
			...
			if (SourceUnit.AffectedByEffectNames.Find('SupportGrenade_FreeAction') != -1 && SourceUnit.ActionPoints.Find('FleetFootedRapidDeployment') == -1)
				SourceUnit.ActionPoints.AddItem('FleetFootedRapidDeployment');
...
DefaultProperties
{
	FreeActionPointType = "FleetFootedEmergencyAid"
	EffectName = "FleetFootedEmergencyAid"
